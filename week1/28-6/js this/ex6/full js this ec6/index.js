    ...
    makeDrink: function (drinkType) {
      ...
      for(let requirements in this.drinkRequirements)
      {
        ...
        if(drinkType===requirements){
            ...
            flag=true;
            if(coffeeShop.beans<this.drinkRequirements[requirements].beanRequirement)
            {
            alert("Sorry, we’re all out of beans!");
            }
            else
            {
            coffeeShop.beans-=this.drinkRequirements[requirements].beanRequirement;
            console.log("the total beans after make dring is : "+ this.beans);
            }
        }
      }
      if(!flag)
      alert("Sorry, we don’t make because "+drinkType +" not in requirements");
    },
    buyBeans:function(numBeans){
      //i decide that the cost of bean equal 3$
      let total =3*numBeans;
      console.log(" the total beans and money before buy bean is :"+this.money +" "+ this.beans);
      if(this.money<total)
      {
      ...
      console.log(" the total beans and money after buy bean is :"+this.money+" "+ this.beans);
      }
    },
